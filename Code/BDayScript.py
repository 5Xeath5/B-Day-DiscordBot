...
async def message():
    ...
    #checks for time and date and sends message when appropriate
...
@lightbulb.implements(lightbulb.SlashCommand)
#Sets up the channel id in which the bot will send the b-day messages
async def setup(ctx):
    guild_channel_dic = FM.guild_channel()
    current_channel = ctx.get_channel()
...
async def delete_user(ctx):
    flag = FM.delete(ctx.options.user)
    ...
    if flag:
        ...
        await ctx.respond("...")
        await bot.rest.create_message(ctx.get_channel(), "Error: User not found in server")
...
async def xlist(ctx):
    ...
    if not flag:
        await bot.rest.create_message(ctx.get_channel(), "Empty")

@bot.command
@lightbulb.option('month', "Birth Month (mm)")
@lightbulb.option('day', 'Birth Day (dd)')
@lightbulb.command('dlist', 'list out users with the given birthdate')
@lightbulb.implements(lightbulb.SlashCommand)
async def dlist(ctx):
    current_channel = ctx.get_channel()
    flag = False
    ...
    await ctx.respond("Yay!")
    for whole_data in nested_data:
        if (str(ctx.get_guild().id) in whole_data and whole_data[0] == ctx.options.month and whole_data[1] == ctx.options.day
          and len(whole_data)) > 1: 
            flag = True
            user = bot.cache.get_user(int(whole_data[2]))
            await bot.rest.create_message(ctx.get_channel(), user.username)
    if not flag:
        await bot.rest.create_message(ctx.get_channel(), "None")

@bot.command
@lightbulb.option('id', "User ID")
@lightbulb.command('ulist', 'provides birthdate of given user')
@lightbulb.implements(lightbulb.SlashCommand)
async def ulist(ctx):
    nested_data = FM.bdaylist()
    flag = False
    ...
    for whole_data in nested_data:
        if (str(ctx.get_guild().id) in whole_data and ctx.options.id in whole_data):
            flag = True
            data = f"Month: {whole_data[0]} Day: {whole_data[1]}"
    if not flag:
        await ctx.respond('...')
        await bot.rest.create_message(ctx.get_channel(), "User not found")
    else:
        await ctx.respond('Yay!')
        await bot.rest.create_message(ctx.get_channel(), data)
